 ...
 */
export const isLocalBuildAvailable = (): boolean => {
  try {
 ...
 */
export const getLocalBuildScripts = (): string[] => {
  try {
    const staticBuildPath = path.join(
      process.cwd(),
      ".storybook/static/local-build/js"
 ...
 */
export const copyLocalBuildToStatic = async (): Promise<boolean> => {
 ...
 */
export const getLocalBuildInfo = (): LocalBuildInfo | null => {
  try {
    const metadataPath = path.join(
      process.cwd(),
      ".storybook/static/local-build/version.json"
 ...
 */
export const isLocalBuildUpToDate = (): boolean => {
 ...
 */
export const getLocalScriptUrl = (scriptName: string): string => {
 ...
 */
export const isLocalBuildAvailableBrowser = async (): Promise<boolean> => {
  try {
    // Try to fetch the version metadata file
    const response = await fetch("/local-build/version.json");
    return response.ok;
  } catch (error) {
    console.error(error);
    return false;
 ...
 */
export const getLocalBuildInfoBrowser =
  async (): Promise<LocalBuildInfo | null> => {
    try {
      const response = await fetch("/local-build/version.json");
      if (!response.ok) {
        return null;
