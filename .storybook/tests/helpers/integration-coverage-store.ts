...
/** `dist/hosted/web/<version>/js` from package.json, or null if unresolvable. */
function getDistJsDir(cwd: string): string | null {
...
/**
 * Directory whose bundles v8-to-istanbul resolves source-map `sources` against.
 *
 * Coverage builds emit maps with `sources` relative to the dist layout
 * (`dist/hosted/web/<version>/js`, e.g. `../../../../../src/...`). `storybook build`
 * copies those bundles into `storybook-static/local-build/js` at a shallower depth
 * (it dereferences the `local-build` symlink), so resolving `sources` there
 * overshoots the repo root and hits land outside `src/`. Resolve against `dist`
 * instead — byte-identical bundles at the depth the maps were baked for — so keys
 * come out as `cwd/src/...` directly. Falls back to the served tree only when `dist`
 * is absent (e.g. an ad-hoc run without a fresh build).
 */
function getMergeJsDir(cwd: string): string {
  const distJs = getDistJsDir(cwd);

  if (distJs && fs.existsSync(distJs)) {
    return distJs;
  }
  ...
  if (fs.existsSync(servedJs)) {
    if (!warnedMergeJsDirFallback) {
      ...
      console.warn(
        "[integration-coverage] dist/hosted/web/<version>/js not found; falling back to storybook-static/local-build/js, whose shallower depth can misresolve source maps. Run npm run playwright:build."
      );
  ...
  throw new Error(
    "[integration-coverage] no merge js dir found (dist/hosted/web/<version>/js or storybook-static/local-build/js); run npm run playwright:build"
  );
}
...
async function mergeLocalBuildHtmlEntry(
  ...
  if (!fs.existsSync(absPath)) {
    /* eslint-disable no-console */
    console.warn(
      "[integration-coverage] internal frame JS missing; use npm run playwright:build (retains *-internal.js):",
      absPath
    );
    /* eslint-enable no-console */
...
export function loadCoverageFromShards(): void {
  ...
  if (Object.keys(coverageMap.toJSON()).length === 0) {
    /* eslint-disable no-console */
    console.warn(
      "[integration-coverage] Istanbul map is empty after loading shards. Check PLAYWRIGHT_INTEGRATION_COVERAGE, Chromium, shard dir, and earlier merge warnings; if bundles are stale, npm run playwright:build.",
      getShardDir()
    );
    /* eslint-enable no-console */
  }
