 ...
 *
 * The success scenario handlers are registered so non-invalid-auth flows
 * (client.create / americanExpress.create) resolve offline and the test
 * reaches the SDK's built-in validation. The "invalid-auth" scenario uses
 * `invalid_tokenization_key`, which the SDK resolves to a GraphQL endpoint
 * whose URL contains "undefined". The handler below intercepts that
 * ClientConfiguration request and returns a 403; for any other endpoint it
 * returns nothing so the success scenario handlers take over.
 */
const invalidAuthHandler = graphql.operation(({ request }) => {
  if (new URL(request.url).pathname.includes("undefined")) {
...
test.describe("American Express - Error Handling", function () {
  test.beforeEach(async ({ getTestUrl, page, network }) => {
    network.use(invalidAuthHandler);
