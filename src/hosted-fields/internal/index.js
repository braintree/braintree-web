...
function initialize(cardForm) {
  ...
  injectWithAllowList(cardForm.configuration.styles, allowedStyles);
...
function createTokenizationHandler(clientInstanceOrPromise, cardForm) {
  return function (options, reply) {
    ...
    Promise.resolve(clientInstanceOrPromise).then(function (client) {
      ...
      var merchantAccountIdForAuthInsight =
        authInsight && authInsight.merchantAccountId;
      ...
      mergedCardData = mergeCardData(
        cardForm.getCardData(fieldsToTokenize),
        options
      );
  ...
  if (status === 403) {
    ...
    if (tokenizationErrorCodes.hasOwnProperty(code)) {
      formattedError = convertToBraintreeError(
        rootError,
        tokenizationErrorCodes[code]
      );
    } else {
      formattedError = new BraintreeError(
        errors.HOSTED_FIELDS_FAILED_TOKENIZATION
      );
      formattedError.details = { originalError: err };
    }
  } else {
    formattedError = new BraintreeError(
      errors.HOSTED_FIELDS_TOKENIZATION_NETWORK_ERROR
    );
    formattedError.details = { originalError: err };
  }
...
function orchestrate(configuration) {
  ...
  var clientPromise = new Promise(function (resolve) {
    window.bus.emit(
      ...
      function (configurationFromMerchantPage) {
  ...
  window.bus.on(events.TOKENIZATION_REQUEST, function (options, reply) {
    var tokenizationHandler = createTokenizationHandler(
      clientPromise,
      cardForm
    );
...
function getSupportedCardBrands(client, merchantConfiguredCardBrands) {
  ...
  Object.keys(merchantConfiguredCardBrands).forEach(function (brand) {
    supportedCardBrands[normalizeCardType(brand)] =
      merchantConfiguredCardBrands[brand];
  });
