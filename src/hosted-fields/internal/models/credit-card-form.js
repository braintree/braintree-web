...
CreditCardForm.prototype.setSupportedCardTypes = function (supportedCardTypes) {
  if (supportedCardTypes) {
    supportedCardTypes = Object.keys(supportedCardTypes).reduce(function (
      brands,
      cardBrand
    ) {
...
CreditCardForm.prototype.emitEvent = function (fieldKey, eventType) {
  ...
  var fields = this._fieldKeys.reduce(
    function (result, key) {
...
CreditCardForm.prototype._onSplitDateChange = function () {
  ...
  if (monthValidation.isValid && yearValidation.isValid) {
    ...
    this.set(
      "expirationMonth.isPotentiallyValid",
      validationResult.isPotentiallyValid
    );
    this.set("expirationYear.isValid", validationResult.isValid);
    this.set(
      "expirationYear.isPotentiallyValid",
      validationResult.isPotentiallyValid
    );
  } else {
    ...
    this.set(
      "expirationMonth.isPotentiallyValid",
      monthValidation.isPotentiallyValid
    );
...
CreditCardForm.prototype.validateField = function (fieldKey) {
  ...
  if (fieldKey === "cvv") {
    validationResult = this._validateCvv(value, {
      minLength:
        this.configuration.fields.cvv &&
        this.configuration.fields.cvv.minlength,
    });
  } else if (fieldKey === "postalCode") {
    validationResult = validate(value, {
      minLength:
        this.configuration.fields.postalCode &&
        this.configuration.fields.postalCode.minlength,
    });
...
function onEmptyChange(form, field) {
  return function () {
    var event = form.get(field + ".isEmpty")
