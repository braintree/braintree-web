...
gulp.task('build:unionpay:frame:html', function (done) {
  ...
  var stream = gulp.src('src/unionpay/internal/unionpay-hosted-fields-frame.html')
...
gulp.task('build:unionpay:frame', gulp.series(
  'build:unionpay:frame:js',
  'build:unionpay:frame:html',
  'build:unionpay:frame:js:delete'
));

gulp.task('build:unionpay', gulp.parallel('build:unionpay:js', 'build:unionpay:frame'));
