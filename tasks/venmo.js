...
var BUILT_QR_CODE_PATH = path.resolve(__dirname, '..', 'src', 'venmo', 'internal', 'vendor', 'node-qrcode.js');

gulp.task('build:venmo:desktop-frame:html', function () {
  var jsFile = fs.readFileSync(DIST_PATH + '/js/venmo-desktop-frame-internal.js', 'utf8');
  var qrCodeFile = fs.readFileSync(BUILT_QR_CODE_PATH, 'utf8');

  var stream = gulp.src(BASE_PATH + '/internal/venmo-desktop-frame.html')
...
gulp.task('build:venmo:desktop-frame', gulp.series(
...
gulp.task('build:venmo', gulp.parallel('build:venmo:js', 'build:venmo:desktop-frame'));
