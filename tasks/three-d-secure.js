...
var HTML_TASKS = FRAMES.map(function (frame) {
  ...
  gulp.task(htmlTaskName, function () {
    ...
    var stream = gulp.src('src/three-d-secure/internal/' + frame + '-frame.html')
...
gulp.task('build:three-d-secure:frame', gulp.series(
  'build:three-d-secure:frame:js',
  'build:three-d-secure:frame:html',
  'build:three-d-secure:frame:js:delete'
));

gulp.task('build:three-d-secure', gulp.parallel('build:three-d-secure:frame', 'build:three-d-secure:js'));
