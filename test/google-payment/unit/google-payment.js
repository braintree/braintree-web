...
describe('GooglePayment', () => {
  ...
  describe('createPaymentDataRequest', () => {
    describe('GooglePay v1 schema', () => {
      it('returns a payment data request object', () => {
        return testContext.googlePayment.createPaymentDataRequest().then(paymentDataRequest => {
      ...
      it('can override parameters with v1 schema overrides and return a v1 schema config', () => {
        return testContext.googlePayment.createPaymentDataRequest({
          merchantId: 'my-id',
      ...
      it('sends an analytics event', () => {
        return testContext.googlePayment.createPaymentDataRequest().then(() => {
          expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.v1.createPaymentDataRequest');
        });
      });
    });
    ...
    describe('GooglePay v2 schema', () => {
      ...
      it('includes a PAYPAL allowedPaymentMethod in v2 if paypal is enabled', () => {
        ...
        return googlePayment.createPaymentDataRequest().then(paymentDataRequest => {
      ...
      it('does not include a merchant id by default', () => {
        return testContext.googlePayment.createPaymentDataRequest({ apiVersion: 2 }).then(paymentDataRequest => {
          expect(paymentDataRequest.merchantInfo).toBeFalsy();
        });
      });

      it('sends an analytics event', () => {
        return testContext.googlePayment.createPaymentDataRequest({ apiVersion: 2 }).then(() => {
          expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.v2.createPaymentDataRequest');
        });
      });
    });
    ...
    describe('when using options during instance construction', () => {
      ...
      it('overrides instantiation options if both are specified', () => {
        ...
        return googlePayment.createPaymentDataRequest({
          merchantInfo: {
  ...
  describe('parseResponse', () => {
    ...
    it('passes back Braintree error when tokenization fails', () => {
      ...
      return expect(testContext.googlePayment.parseResponse(rawResponse)).rejects.toMatchObject({
    ...
    it('sends analytics events when google-payment payment method nonce payload is parsed', () => {
      ...
      return testContext.googlePayment.parseResponse(rawResponse).then(() => {
        expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.parseResponse.succeeded');
        expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.parseResponse.succeeded.google-payment');
      });
    });
    ...
    it('sends analytics events when paypal payment method nonce payload is parsed', () => {
      ...
      return testContext.googlePayment.parseResponse(rawResponse).then(() => {
        expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.parseResponse.succeeded');
        expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.parseResponse.succeeded.paypal');
      });
    });
    ...
    it('sends an analytics event when tokenization results in an error', () => {
      ...
      return Promise.resolve().then(() => {
        return expect(testContext.googlePayment.parseResponse(rawResponse)).rejects
          .toThrow('There was an error when tokenizing the Google Pay payment method.');
      }).then(() => {
        expect(analytics.sendEvent).toHaveBeenCalledWith(expect.anything(), 'google-payment.parseResponse.failed');
      });
    });
  });
