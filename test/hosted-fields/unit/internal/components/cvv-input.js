...
describe('CVV Input', () => {
  ...
  describe('setup', () => {
    ...
    describe('element', () => {
      ...
      it('has autocomplete cc-csc', () => {
        expect(testContext.input.element.getAttribute('autocomplete')).toBe('cc-csc');
      });
      ...
      it('sets the maxLength to custom maxlength if one is provided and is less than 4', () => {
        ...
        jest.spyOn(BaseInput.prototype, 'getConfiguration').mockReturnValue({ maxlength: 3 });
  ...
  describe('setInputState', () => {
    ...
    it('sets the maxlength on possibleCardTypes change', () => {
      ...
      expect(testContext.input.formatter.setPattern).toHaveBeenCalledWith('{{999999}}');

      testContext.input.model.set('possibleCardTypes', []);

      expect(testContext.input.element.getAttribute('maxlength')).toBe('4');
      expect(testContext.input.formatter.setPattern).toHaveBeenCalledWith('{{9999}}');
    });

    it('does not set the maxlength on possibleCardTypes change if a custom maxlength is set', () => {
      let input;

      jest.spyOn(BaseInput.prototype, 'getConfiguration').mockReturnValue({ maxlength: 2 });
