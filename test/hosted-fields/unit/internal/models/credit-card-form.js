...
describe("credit card model", () => {
  ...
  beforeEach(() => {
    ...
    testContext.card = new CreditCardForm(
  ...
  describe("constructor()", () => {
    ...
    describe("_fieldKeys", () => {
      ...
      it("assigns only allowed fields", () => {
        const cardForm = new CreditCardForm(getModelConfig(["number", "foo"]));
    ...
    it("attaches change events for each field", () => {
      const configuration = getModelConfig(["number", "cvv", "expirationDate"]);
      const cardForm = new CreditCardForm(configuration);
      const { calls } = cardForm.on.mock;
  ...
  describe("resetAttributes", () => {
    ...
    it("sets expiration month to current month if using a <select> and no placeholder", () => {
      ...
      expect(
        CreditCardForm.prototype.resetAttributes.call(testContext.scope)
          .expirationMonth
      ).toEqual({
        value: currentMonth,
        isFocused: false,
    ...
    it("sets expiration month to empty if using a <select> and a placeholder", () => {
      ...
      expect(
        CreditCardForm.prototype.resetAttributes.call(testContext.scope)
          .expirationMonth
      ).toEqual(testContext.emptyProperty);
    });
  ...
  describe("emitEvent", () => {
      ...
      CreditCardForm.prototype.emitEvent.call(
        {
          ...
          _fieldKeys: ["number", "cvv", "expirationDate"],
        },
        "number",
        "foo"
      );
    ...
    it("emits CARD_FORM_ENTRY_HAS_BEGUN when field is focused the first time", () => {
      const card = new CreditCardForm(
        getModelConfig(["cvv", "expirationMonth", "expirationYear"])
      );
  ...
  describe("getCardData", () => {
    ...
    it("skips CVV if not in the config", () => {
      const card = new CreditCardForm(
        getModelConfig(["number", "expirationMonth", "expirationYear"])
      );
    ...
    it("gets cardholder name if specified in the config", () => {
      const card = new CreditCardForm(
        getModelConfig(["number", "cardholderName"])
      );
    ...
    it("can get expiration month and year if expirationDate is not specified", () => {
      let cardData;
      const card = new CreditCardForm(
        getModelConfig(["number", "cvv", "expirationMonth", "expirationYear"])
      );
  ...
  describe("invalidFieldKeys", () => {
    it("returns invalid keys when all fields are invalid", () => {
      ...
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.arrayContaining(["number"])
      );
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.arrayContaining(["cvv"])
      );
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.arrayContaining(["expirationDate"])
      );
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.arrayContaining(["postalCode"])
      );
    });
    ...
    it("returns only invalid keys when some keys are invalid", () => {
      ...
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.not.arrayContaining(["cvv"])
      );
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.arrayContaining(["expirationDate"])
      );
    ...
    it("marks postal code as invalid if set minlength is not reached", () => {
      ...
      expect(testContext.card.invalidFieldKeys()).toEqual(
        expect.not.arrayContaining(["postalCode"])
      );
    ...
    it("marks cvv as invalid if set minlength is not reached in cvv only integration", () => {
      const card = new CreditCardForm(getModelConfig(["cvv"]));

      card.set("cvv.value", "123");

      expect(card.invalidFieldKeys()).toEqual(
        expect.not.arrayContaining(["cvv"])
      );
    ...
    it("ignores keys that do not exist within card form", () => {
      ...
      result = testContext.card.invalidFieldKeys([
        "foo",
        "number",
        "bar",
        "postalCode",
  ...
  describe("possibleCardTypes", () => {
    ...
    it("emits a CARD_TYPE_CHANGE event", () => {
      ...
      expect(testContext.card.emitEvent).toHaveBeenCalledWith(
        "number",
        externalEvents.CARD_TYPE_CHANGE
      );

      for (i = 0; i < testContext.card.emitEvent.mock.calls.length; i++) {
        if (
          testContext.card.emitEvent.mock.calls[i][1] ===
          externalEvents.CARD_TYPE_CHANGE
        ) {
          callCount++;
        }
    ...
    it("emits a VALIDITY_CHANGE event", () => {
      ...
      expect(testContext.card.emitEvent).toHaveBeenCalledWith(
        "number",
        externalEvents.VALIDITY_CHANGE
      );
    ...
    describe("supporting card types", () => {
      ...
      it("sets supported property for all card types", () => {
        const possibleCardTypes =
          testContext.supportedCardForm.get("possibleCardTypes");
      ...
      it("sets number to potentially valid after removing an unsupported card number", () => {
        ...
        expect(
          testContext.supportedCardForm.get("number.isPotentiallyValid")
        ).toBe(false);
      ...
      it("is not valid nor potentially valid when using an unsupported card type", () => {
        ...
        expect(testContext.supportedCardForm.get("number.value")).toBe(
          mastercard
        );
        expect(testContext.supportedCardForm.get("number.isValid")).toBe(false);
        expect(
