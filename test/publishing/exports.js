...
const components = require('../helpers/components').components.reduce((result, component) => {
  result[component] = {
...
const componentNames = Object.keys(components).map(key => key.replace(/-./g, str => str[1].toUpperCase()));
...
describe('exports', () => {
  describe.each([
    ...
    it(`exports components for ${moduleName}`, () =>{
      expect(Object.keys(distFile)).toStrictEqual(expect.arrayContaining(componentNames));
    });
  });
