...
describe("GraphQL", () => {
  ...
  describe("getHeaders", () => {
    ...
    it("sends an analytics event for tokenization key", () => {
      ...
      expect(analyticsEvents).toStrictEqual(
        expect.arrayContaining(["graphql.tokenization-key"])
      );
      expect(analyticsEvents).not.toStrictEqual(
        expect.arrayContaining(["graphql.authorization-fingerprint"])
      );
    });
  ...
  describe("getBody", () => {
    it("creates a GraphQL mutation for credit card tokenization", () => {
      ...
      expect(parsedBody.query).toStrictEqual(
        expect.stringContaining("tokenizeCreditCard")
      );
    ...
    it("creates a GraphQL mutation for credit card tokenization with authentication insight in the query when an authentication insight param is passed in the body", () => {
      ...
      expect(parsedBody.query).toStrictEqual(
        expect.stringContaining("authenticationInsight")
      );
      expect(parsedBody.query).toStrictEqual(
        expect.stringContaining("customerAuthenticationRegulationEnvironment")
      );
      expect(parsedBody.variables).toBeDefined();
    ...
    it("creates a GraphQL mutation for credit card tokenization without authentication insight in the query when no authentication insight param is passed in the body", () => {
      ...
      expect(parsedBody.query).not.toStrictEqual(
        expect.stringContaining("authenticationInsight")
      );
      expect(parsedBody.query).not.toStrictEqual(
        expect.stringContaining("merchantAccountId")
      );
      expect(parsedBody.query).not.toStrictEqual(
        expect.stringContaining("customerAuthenticationRegulationEnvironment")
      );
    ...
    it("creates a GraphQL query for configuration", () => {
      ...
      expect(parsedBody.query).toStrictEqual(
        expect.stringContaining("clientConfiguration")
      );
  ...
  describe("adaptResponseBody", () => {
    ...
    it("remaps card brand codes", () => {
      ...
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("MASTERCARD"))
          .creditCards[0].details.cardType
      ).toBe("MasterCard");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("DINERS")).creditCards[0]
          .details.cardType
      ).toBe("Discover");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("DISCOVER"))
          .creditCards[0].details.cardType
      ).toBe("Discover");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("INTERNATIONAL_MAESTRO"))
          .creditCards[0].details.cardType
      ).toBe("Maestro");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("UK_MAESTRO"))
          .creditCards[0].details.cardType
      ).toBe("Maestro");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("JCB")).creditCards[0]
          .details.cardType
      ).toBe("JCB");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("UNION_PAY"))
          .creditCards[0].details.cardType
      ).toBe("UnionPay");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("VISA")).creditCards[0]
          .details.cardType
      ).toBe("Visa");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("ELO")).creditCards[0]
          .details.cardType
      ).toBe("Elo");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("HIPER")).creditCards[0]
          .details.cardType
      ).toBe("Hiper");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("HIPERCARD"))
          .creditCards[0].details.cardType
      ).toBe("Hipercard");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("SOLO")).creditCards[0]
          .details.cardType
      ).toBe("Unknown");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse("UNKNOWN")).creditCards[0]
          .details.cardType
      ).toBe("Unknown");
      expect(
        graphQLRequest.adaptResponseBody(makeResponse()).creditCards[0].details
          .cardType
      ).toBe("Unknown");
    });
  ...
  describe("determineStatus", () => {
    it("returns 200 for successful responses", () => {
      ...
      expect(graphQLRequest.determineStatus(200, fakeGraphQLResponse)).toBe(
        200
      );
    });
    ...
    it("returns 403 for field coercion errors", () => {
      ...
      const fakeGraphQLResponse = {
        ...
        errors: [
          {
            message:
              "Variable 'billingAddress' has an invalid value. Expected type 'Map' but was 'String'. Variables for input objects must be an instance of type 'Map'.",
            locations: [
              {
                line: 1,
                column: 29,
    ...
    it("returns 403 for validation errors", () => {
      ...
      const fakeGraphQLResponse = {
        ...
        errors: [
          {
            message:
              "Validation error of type FieldUndefined: Field 'cardType' in type 'TokenizeCreditCard' is undefined",
            locations: [
              {
                line: 1,
                column: 29,
    ...
    it("returns 500 when status code is not present", () => {
      ...
      expect(graphQLRequest.determineStatus(null, fakeGraphQLResponse)).toBe(
        500
      );
    });
    ...
    it("sends an analytics event for actual and determined http status", () => {
      ...
      expect(analyticsEvents).toStrictEqual(
        expect.arrayContaining(["graphql.status.200"])
      );
      expect(analyticsEvents).toStrictEqual(
        expect.arrayContaining(["graphql.determinedStatus.403"])
      );
    });
  });
});
