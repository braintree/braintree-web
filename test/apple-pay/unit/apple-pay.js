...
describe('ApplePay', () => {
  ...
  it('exposes a readonly merchantIdentifier property', () => {
    ...
    expect(testContext.applePay.merchantIdentifier).toBe('com.example.test-merchant-identifier');
    ...
    expect(err.message).toBe('Cannot assign to read only property \'merchantIdentifier\' of object \'#<ApplePay>\'');
    expect(testContext.applePay.merchantIdentifier).toBe('com.example.test-merchant-identifier');
  });
  ...
  describe('createPaymentRequest', () => {
    ...
    it('leaves properties intact', () => {
      expect(testContext.applePay.createPaymentRequest({ foo: 'boo' }).foo).toBe('boo');
    });

    it('applies countryCode when undefined', () => {
      expect(testContext.applePay.createPaymentRequest({}).countryCode).toBe('decorated');
    });

    it('applies currencyCode when undefined', () => {
      expect(testContext.applePay.createPaymentRequest({}).currencyCode).toBe('decorated');
    });

    it('applies gateway merchantCapabilities when not defined on argument but defined on gateway', () => {
      expect(testContext.applePay.createPaymentRequest({}).merchantCapabilities).toEqual(['decorated']);
    });
    ...
    it('applies default merchantCapabilities when not defined on argument and gateway', () => {
      ...
      expect(testContext.applePay.createPaymentRequest({}).merchantCapabilities).toEqual(['supports3DS']);
    });

    it('applies supportedNetworks when undefined', () => {
      expect(testContext.applePay.createPaymentRequest({}).supportedNetworks).toEqual(['visa', 'amex', 'masterCard']);
    });

    it('does not apply countryCode when defined', () => {
      expect(testContext.applePay.createPaymentRequest(testContext.paymentRequest).countryCode).toBe('defined');
    });

    it('does not apply currencyCode when defined', () => {
      expect(testContext.applePay.createPaymentRequest(testContext.paymentRequest).currencyCode).toBe('defined');
    });

    it('does not apply merchantCapabilities when defined', () => {
      expect(testContext.applePay.createPaymentRequest(testContext.paymentRequest).merchantCapabilities).toEqual(['defined']);
    });

    it('does not apply supportedNetworks when defined', () => {
      expect(testContext.applePay.createPaymentRequest(testContext.paymentRequest).supportedNetworks).toEqual(['defined']);
    });
  });
  ...
  describe('performValidation', () => {
    ...
    it('rejects with error when event is undefined', () =>
      testContext.applePay.performValidation(false).catch(err => {
        ...
        expect(err.message).toBe('performValidation must be called with a validationURL.');
      }));

    it('rejects with error when event.validationURL is undefined', () =>
      testContext.applePay.performValidation({ some: 'property' }).catch(err => {
        expect(err).toBeInstanceOf(BraintreeError);
        expect(err.code).toBe('APPLE_PAY_VALIDATION_URL_REQUIRED');
        expect(err.type).toBe('MERCHANT');
        expect(err.message).toBe('performValidation must be called with a validationURL.');
    ...
    it('overrides merchantIdentifier from the options', () =>
      testContext.applePay.performValidation({
    ...
    it.each([
      ['an invalid request', new BraintreeError({ type: 'NETWORK', code: 'CLIENT_REQUEST_ERROR', message: 'This is a fake client error', details: { originalError: new Error('Validation failed') }}), { code: 'APPLE_PAY_MERCHANT_VALIDATION_FAILED', type: 'MERCHANT', message: 'Make sure you have registered your domain name in the Braintree Control Panel.', originalMessage: 'Validation failed' }],
      ['a network error', new BraintreeError({ type: 'NETWORK', code: 'CLIENT_REQUEST_TIMEOUT', message: 'This is a fake client error' }), { code: 'APPLE_PAY_MERCHANT_VALIDATION_NETWORK', type: 'NETWORK', message: 'A network error occurred when validating the Apple Pay merchant.', originalMessage: 'This is a fake client error' }]
    ])('makes a request to the gateway and rejects if it fails due to %s', (n, requestError, expectedError) => {
      ...
      testContext.client.request = options => {
        ...
        expect(options.data.applePayWebSession.merchantIdentifier).toBe('com.example.test-merchant-identifier');
        expect(options.data.applePayWebSession.domainName).toBe('localhost');
        expect(options.data.applePayWebSession.displayName).toBe('Awesome Merchant');
        expect(options.data.applePayWebSession.validationUrl).toBe('https://apple-pay-gateway-cert.apple.com/paymentservices/startSession');
    ...
    it('makes a request to the gateway and resolves if it succeeds', () => {
      ...
      return testContext.applePay.performValidation(validationOptions).then(response => {
  ...
  describe('tokenize', () => {
    ...
    it('rejects with error when token is undefined', () =>
      testContext.applePay.tokenize(false).catch(err => {
        ...
        expect(err.type).toBe('MERCHANT');
        expect(err.message).toBe('tokenize must be called with a payment token.');
      }));
    ...
    it('makes a request to the gateway and rejects if it fails', () => {
      ...
      return testContext.applePay.tokenize({ token: token }).catch(err => {
        ...
        expect(err.message).toBe('A network error occurred when processing the Apple Pay payment.');
      });
    });
    ...
    it('makes a request to the gateway and resolves if it succeeds', () => {
      ...
      return testContext.applePay.tokenize({
        token: payment.token
      }).then(response => {
    ...
    describe('performValidation', () => {
      it('submits succeeded', () => {
        ...
        return ApplePay.prototype.performValidation.call({
          ...
          expect(analytics.sendEvent).toHaveBeenCalledWith(testContext.client, 'applepay.performValidation.succeeded');
        });
      });
      ...
      it('submits failed', () => {
        ...
        return ApplePay.prototype.performValidation.call({
          ...
          expect(analytics.sendEvent).toHaveBeenCalledWith(testContext.client, 'applepay.performValidation.failed');
        });
      });
    });
    ...
    describe('tokenize', () => {
      it('submits succeeded', () => {
        ...
        return ApplePay.prototype.tokenize.call({
          ...
          expect(analytics.sendEvent).toHaveBeenCalledWith(testContext.client, 'applepay.tokenize.succeeded');
        });
      });
      ...
      it('submits failed', () => {
        ...
        return ApplePay.prototype.tokenize.call({
          ...
          expect(analytics.sendEvent).toHaveBeenCalledWith(testContext.client, 'applepay.tokenize.failed');
        });
      });
    });
  });
