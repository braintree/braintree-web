...
describe('createAssetsUrl', () => {
  ...
  it('creates an assets url for production when client token does not include environment', () => {
    expect(create(fake.clientTokenWithoutEnvironment)).toBe('https://assets.braintreegateway.com');
  });

  it('creates an assets url from a sandbox tokenization key', () => {
    expect(create('sandbox_testing_merchant')).toBe('https://assets.braintreegateway.com');
  });

  it('creates an assets url from a production tokenization key', () => {
    expect(create('production_testing_merchant')).toBe('https://assets.braintreegateway.com');
  });
});
