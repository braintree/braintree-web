...
describe("querystring", () => {
  describe("hasQueryParams", () => {
    it("returns true when url has query params", () => {
      expect(querystring.hasQueryParams("https://example.com?foo=bar")).toBe(
        true
      );
    });
  ...
  describe("queryify", () => {
    ...
    it("returns a url with params if given a nested params object", () => {
      ...
      expect(querystring.queryify(url, params)).toBe(
        url + encodeURI("?foo[bar][baz]=qux")
      );
    });
  ...
  describe("stringify", () => {
    it("turns an object into a GET string", () => {
      ...
      expect(querystring.stringify(params)).toBe(
        "hello=1&world=some%20string&integer=1&dbl=2"
      );
    });

    it("encodes an array as a url encoded string", () => {
      const params = {
        myArray: [0, "second", { ordinal: "third" }],
      };

      expect(querystring.stringify(params)).toEqual(
        "myArray%5B%5D=0&myArray%5B%5D=second&myArray%5B%5D%5Bordinal%5D=third"
      );
    });
    ...
    it("encodes a nested object as a url encoded string", () => {
      ...
      expect(querystring.stringify(params)).toEqual(
        "topLevel=1&nested%5Bhello%5D=2&nested%5Bworld%5D=3&nested%5BnestedNested%5D%5B1%5D=1"
      );
    });
    ...
    it("does not include prototyped methods in encoded string", () => {
      ...
      expect(querystring.stringify(new MyClass())).not.toMatch(
        /shouldNotBeIncluded/i
      );
    });
  });
  ...
  describe("parse", () => {
    it("returns empty object when no query params", () => {
      expect(
        Object.keys(querystring.parse("https://example.com"))
      ).toHaveLength(0);
    });

    it("does not return hash data", () => {
      expect(querystring.parse("https://example.com?var=a#something").var).toBe(
        "a"
      );
    });

    it("handles a querystring with multiple variables", () => {
      const actual = querystring.parse(
        "https://example.com?var=a&var2=b#something"
      );
    ...
    it("handles a querystring with a hash in the base url before the query params", () => {
      expect(querystring.parse("https://example.com/#/mobile?var=a").var).toBe(
        "a"
      );
    });
  });
