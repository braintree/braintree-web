...
describe('isVerifiedDomain', () => {
  ...
  it('returns false if a invalid localhost domain is used', () => {
    ...
    expect(isVerifiedDomain('https://braintreepayments.com.localhost.com:3099')).toBe(false);
  });
  ...
  it('returns true if a braintree domain with a port is used', () => {
    expect(isVerifiedDomain('https://www.braintreepayments.com:3000')).toBe(true);
    expect(isVerifiedDomain('https://www.braintreegateway.com:3000')).toBe(true);
  ...
  it('returns true if a braintree domain with multiple subdomains is used', () => {
    expect(isVerifiedDomain('https://foo.bar.baz.braintreepayments.com')).toBe(true);
    expect(isVerifiedDomain('https://foo.bar.baz.braintreegateway.com')).toBe(true);
    expect(isVerifiedDomain('https://foo.bar.baz.braintree-api.com')).toBe(true);
  });

  it('returns false if a braintree domain over http is used', () => {
    expect(isVerifiedDomain('http://www.braintreepayments.com:3000')).toBe(false);
    expect(isVerifiedDomain('http://www.braintreegateway.com:3000')).toBe(false);
  ...
  it('returns false if a non-braintree non-paypal domain is used with braintree in the path', () => {
    expect(isVerifiedDomain('https://braintreepayments.com.example.com')).toBe(false);
    expect(isVerifiedDomain('https://google.com/www.braintreepayments.com')).toBe(false);
  });
});
