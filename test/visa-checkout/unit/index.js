...
const { fake: { client: fakeClient, clientToken, configuration: fakeConfiguration }} = require('../../helpers');
...
describe('visaCheckout.create', () => {
  ...
  it('verifies with basicComponentVerification', () =>
    create({ client: testContext.client }).then(() => {
      ...
      expect(basicComponentVerification.verify).toBeCalledWith({ name: 'Visa Checkout', client: testContext.client });
    }));

  it('resolves with a VisaCheckout instance when successful', () => {
    expect(create({ client: testContext.client })).resolves.toBeInstanceOf(VisaCheckout);
  });
  ...
  it('rejects with an error when visa checkout is not enabled in configuration', () => {
    ...
    return create({ client: testContext.client }).catch(err => {
      ...
      expect(err.message).toBe('Visa Checkout is not enabled for this merchant.');
    });
  });

  it('sends an analytics event', () =>
    create({ client: testContext.client }).then(() => {
      expect(analytics.sendEvent).toBeCalledWith(testContext.client, 'visacheckout.initialized');
    }));
});
